...
public class RNGethModule extends ReactContextBaseJavaModule {
  ...
  public void newAccount(String passphrase, Promise promise) {
      try {
          Account acc = this.getKeyStore().newAccount(passphrase);
  ...
  public void balanceAccount(Promise promise) {
      try {
          ...
          if( acc != null ) {
              ...
              BigInt balance = this.getNode().getEthereumClient()
  ...
  public void updateAccount(String oldPassphrase, String newPassphrase, Promise promise) {
      try {
          Account acc = this.getAccount();
          if( acc != null ) {
              this.getKeyStore().updateAccount(acc, oldPassphrase, newPassphrase);
              promise.resolve(true);
          } else {
  ...
  public void deleteAccount(String passphrase, Promise promise) {
      try {
          Account acc = this.getAccount();
          if( acc != null ) {
              this.getKeyStore().deleteAccount(acc, passphrase);
              promise.resolve(true);
          } else {
   ...
   */
  @ReactMethod
  public void exportKey(String creationPassphrase, String exportPassword, Promise promise) {
      try {
          Account acc = this.getAccount();
          if( acc != null ) {
              byte[] key = this.getKeyStore().exportKey(acc, creationPassphrase, exportPassphrase);
              promise.resolve(key);
          } else {
  ...
  public void importKey(byte[] key, String oldPassphrase, String newPassphrase, Promise promise) {
      try {
          Account acc = this.getKeyStore().importKey(key, oldPassphrase, newPassphrase);
          promise.resolve(acc);
      } catch (Exception e) {
